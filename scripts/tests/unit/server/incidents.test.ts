...
function incidentDb(): Database.Database {
...
test("Incident grouping caps stored detail and issues public ids", () => {
  const db = incidentDb();
...
test("client blob stacks with different random ids still group together after symbolication", () => {
  const db = incidentDb();
