...
function openIncidentMemory(): Database.Database {
...
test("each occurrence gets a public id while detail is capped at ten", () => {
  const db = openIncidentMemory();
...
test("client blob URLs with different ids fingerprint as the same failure", () => {
  const db = openIncidentMemory();
