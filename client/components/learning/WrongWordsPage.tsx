...
interface WrongWordsPageProps {
  onBack: () => void;
  onNavigate: (view: "clear-wrong") => void;
}
...
export default function WrongWordsPage({
  ...
  const [words, setWords] = useState<WordWithWrongCount[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const data = await fetchWrongWords(page * PAGE_SIZE, PAGE_SIZE);
      setWords(data.words || []);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button
          variant="contained"
          sx={{ ml: 2, borderRadius: 20, fontSize: "0.8rem" }}
          onClick={() => onNavigate("clear-wrong")}
        >
          清错词
        </Button>
      </Box>
