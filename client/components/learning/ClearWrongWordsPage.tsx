...
interface ClearWrongWordsPageProps {
  onBack: () => void;
...
export default function ClearWrongWordsPage({
  onBack,
  onProgress,
}: ClearWrongWordsPageProps) {
  ...
  const fetchNext = useCallback(async () => {
    const data = await fetchNextWrongWord();
    if (data.word) {
      ...
      setOptions(
        distractors.length > 0 ? options.sort(() => Math.random() - 0.5) : [],
      );
      setShowOptions(distractors.length > 0);
  ...
  useEffect(() => {
    if (currentWord && distractors.length > 0) {
      timerRef.current = setTimeout(() => {
        setHighlightedCorrect(true);
        handlePractice(false, false);
      }, 5000);
    }
  }, [currentWord, distractors, handlePractice]);
  ...
  const getExitMessage = () => {
    ...
    if (total === 0) return "本次清错你还没有开始哦~";
    const accuracy = total > 0 ? Math.round((correct / total) * 100) : 0;
    if (accuracy === 100) {
      return `！？强强？！本次清错你复习了${total}个单词，居然全部答对了！好吧我承认你很牛逼，只比原神差了一点点`;
    }
    return `你个肺雾😡才复习了${total}个单词，居然连全对都做不到，仅仅只有${accuracy}%的正确率，你不配和神使用同一个暗网`;
  };
